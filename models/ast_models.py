        ...
        if show_mask == False:
            ...
            for i in range(B):
                result = [
                    float(t) * 99
                    for t in torch.eq(
                        torch.argmax(self.softmax(total), dim=0), self.mask_correct
                    )
                ]
                pred[i, mask_index[i], :] = (
                    torch.tensor(result).reshape(mask_patch, 1).expand(mask_patch, 256)
                )
...
if __name__ == "__main__":
    ...
    input_tdim = (
        100  # fine-tuning data length can be different with pretraining data length
    )
    ast_mdl = ASTModel(
        label_dim=35,
        fshape=16,
        ...
        load_pretrained_mdl_path="./test_mdl.pth",
    )
