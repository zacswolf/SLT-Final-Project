...
parser.add_argument("--data_new", type=str, default=None, help="the dataset is new  or not")
parser.add_argument("--data_id", type=str, default='128mel_16frames_0.1sec', help="load the data with corresponding id")
...
if args.bal == 'bal':
    if args.data_new == 'new':
            train_feats = np.load("../../../SLT-Final-Project/data/dataset_" + args.data_id + "/train_feats_" + args.data_id + ".npy", allow_pickle=True)
            train_labels = np.load("../../../SLT-Final-Project/data/dataset_" + args.data_id + "/train_labels_" + args.data_id + ".npy", allow_pickle=True)
            train_feats_tensor = torch.tensor(train_feats, dtype=torch.float)
            train_labels_tensor = torch.tensor(train_labels, dtype=torch.float)
            train_dataset = torch.utils.data.TensorDataset(train_feats_tensor,train_labels_tensor)
            train_loader = torch.utils.data.DataLoader(train_dataset, args.batch_size, shuffle = True, num_workers=args.num_workers, pin_memory=False, drop_last = True)
...
#help
if args.data_new == 'new':
    val_feats = np.load("../../../SLT-Final-Project/data/dataset_" + args.data_id + "/val_feats_" + args.data_id + ".npy", allow_pickle=True)
    val_labels = np.load("../../../SLT-Final-Project/data/dataset_" + args.data_id + "/val_labels_" + args.data_id + ".npy", allow_pickle=True)
    val_feats_tensor = torch.tensor(val_feats, dtype=torch.float)
...
if args.data_eval != None:
    ...
    eval_feats = np.load("../../../SLT-Final-Project/data/dataset_" + args.data_id + "/test_feats_" + args.data_id + ".npy", allow_pickle=True)
    eval_labels = np.load("../../../SLT-Final-Project/data/dataset_" + args.data_id + "/test_labels_" + args.data_id + ".npy", allow_pickle=True)
    eval_feats_tensor = torch.tensor(eval_feats, dtype=torch.float)
    eval_labels_tensor = torch.tensor(eval_labels, dtype=torch.float)

    eval_dataset = torch.utils.data.TensorDataset(eval_feats_tensor, eval_labels_tensor)
    eval_loader = torch.utils.data.DataLoader(eval_dataset, batch_size = args.batch_size*2, shuffle = False, num_workers = args.num_workers, pin_memory = True)
